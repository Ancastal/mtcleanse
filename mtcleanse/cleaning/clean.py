...
class TextCleaner:
    ...
    def _detect_domain_outliers(
        ...
        if not self.config.enable_domain_filtering:
            return [True] * len(
                source_texts
            )  # Return all as inliers if domain filtering is disabled
    ...
    def _detect_length_outliers(
        ...
        logger.info(
            f"Source - Mean: {src_lengths.mean():.1f}, Std: {src_lengths.std():.1f}, "
            f"Min: {src_lengths.min()}, Max: {src_lengths.max()}"
        )
        logger.info(
            f"Target - Mean: {tgt_lengths.mean():.1f}, Std: {tgt_lengths.std():.1f}, "
    ...
    def _clean_single_text(self, text: str) -> Tuple[str, Dict[str, bool]]:
        ...
        flags = {"empty": False, "too_short": False, "too_long": False}
    ...
    def clean_parallel_texts(
        ...
        logger.info(
            f"Performing statistical outlier detection on {len(temp_source)} pairs..."
        )
        ...
        if self.config.enable_domain_filtering:
            ...
            final_mask = [
                a and b for a, b in zip(length_inlier_mask, domain_inlier_mask)
            ]
        else:
        ...
        cleaned_source = [
            text for text, is_inlier in zip(temp_source, final_mask) if is_inlier
        ]
        cleaned_target = [
            text for text, is_inlier in zip(temp_target, final_mask) if is_inlier
        ]
    ...
    def clean_file(
        ...
        with open(source_file, "r", encoding="utf-8") as src_f, open(
        ...
        filtered_pairs = [
            (src, tgt) for src, tgt in original_pairs if (src, tgt) not in cleaned_pairs
        ]
        ...
        with open(output_source, "w", encoding="utf-8") as src_f, open(
            output_target, "w", encoding="utf-8"
        ) as tgt_f:
        ...
        if filtered_source and filtered_target:
            ...
            logger.info(
                f"Writing filtered files: {filtered_source} and {filtered_target}"
            )
            with open(filtered_source, "w", encoding="utf-8") as src_f, open(
                filtered_target, "w", encoding="utf-8"
            ) as tgt_f:
        ...
        if json_output:
            ...
            json_data = []
            for src, tgt in zip(cleaned_source, cleaned_target):
                entry = {"instruction": instruction or "", "input": src, "target": tgt}
